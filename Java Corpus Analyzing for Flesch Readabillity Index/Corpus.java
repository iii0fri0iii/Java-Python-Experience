...
public class Corpus {
    ...
    /**
     * Helper method to determine if the input word is the last in a sentence (ie
     * word
     * ends with '.' , '?' , '!' , ':' , or ';'
     * 
    ...
    /**
     * Determine the Flesch readability index of this Corpus. Values near 100
     * indicate easily
     * read material (comics). Values near 0 indicate difficult-to-read material
     * (insurance
     * policies). The index is based on ratios of the number of sentences, words,
     * and
     * syllables in the corpus. No linguistic analysis is made.
     * 
     * @return the Flesch readability index of this Corpus
     * @throws FileNotFoundException        if the file used for this Corpus is not
     *                                      found
    ...
    public int getFleschIndex() throws FileNotFoundException,
        ...
        int numWords = 0;
        int numSentences = 0;
        BufferedReader src = new BufferedReader(new InputStreamReader(new FileInputStream(inputFile), encoding));
        while ((line = src.readLine()) != null) {
            line = line.trim(); // must trim before splitting
            if (line.length() != 0) {
                fields = line.split("\\s+");
                for (int i = 0; i < fields.length; i++) {
                    if (endOfSentence(fields[i]))
                        numSentences += 1;
                    numWords += 1;
                    numSyllables += getNumSyllables(fields[i]);
